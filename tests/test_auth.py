...
def register_and_login(username: str, password: str) -> str:
    ...
    resp = client.post(
        "/auth/token-json", json={"username": username, "password": password}
    )
    assert resp.status_code == 200
...
def test_register_then_login_me():
    resp = client.post(
        "/auth/register", json={"username": "alice", "password": "wonderland"}
    )
    assert resp.status_code == 200
    data = resp.json()
    ...
    resp = client.post(
        "/auth/token-json", json={"username": "alice", "password": "wonderland"}
    )
    assert resp.status_code == 200
...
def test_notify_test_dry_run_ok():
    ...
    resp = client.post(
        "/auth/me/notify-test", headers={"Authorization": f"Bearer {token}"}
    )
    assert resp.status_code == 200
    data = resp.json()
